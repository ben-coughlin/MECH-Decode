...
import static org.firstinspires.ftc.simteamcode.RobotPosition.AngleWrap;
import static org.firstinspires.ftc.simteamcode.RobotPosition.subtractAngles;
...
//import com.qualcomm.robotcore.util.Range; // ALEJANDRO
import org.firstinspires.ftc.simteamcode.Range; // ALEJANDRO
...
//        if (DEBUG_MOVEMENT) {
//            clientSim.sendText(String.format(Locale.US, "[Mov] gunTo: T(%.1f,%.1f) A(%.1f)", targetX, targetY, Math.toDegrees(point_angle)));
////            clientSim.sendLine(worldXPosition, worldYPosition, targetX, targetY);
////            System.out.println(String.format(Locale.US, "[GUN_TO_POS] Target(%.2f, %.2f), PointAngle(%.2f rad), MoveSpeed(%.2f), PointSpeed(%.2f), Stop(%b)",
////                    targetX, targetY, point_angle, movement_speed, point_speed, stop));
////            System.out.println(String.format(Locale.US, "[GUN_TO_POS] Robot At(%.2f, %.2f, Angle: %.2f rad)", worldXPosition, worldYPosition, worldAngle_rad));
//        }
...
//        if (DEBUG_MOVEMENT) {
//            System.out.println(String.format(Locale.US, "[GUN_TO_POS] RelXYToPoint(%.2f, %.2f), InitialPowersXY(%.2f, %.2f)",
//                    relative_x_to_point, relative_y_to_point, movement_x_power, movement_y_power));
//        }
...
//        if (DEBUG_MOVEMENT) {
//            System.out.println(String.format(Locale.US, "[GUN_TO_POS] ClippedMoveXY(%.2f, %.2f) with SpeedLimit(%.2f)", movement_x, movement_y, movement_speed));
//        }
...
//        if (DEBUG_MOVEMENT) {
//            System.out.println(String.format(Locale.US, "[GUN_TO_POS] FinalScaledMoveXY(%.3f, %.3f) by TurnErrorFactor(%.3f)",
    ...
    public static boolean followCurve(ArrayList<CurvePoint> allPoints, double followAngle, double tolerance) {
        ...
        if (DEBUG_MOVEMENT) {
            clientSim.sendLine(
                    ...
                    clippedToPath.x,          // Clipped point X
